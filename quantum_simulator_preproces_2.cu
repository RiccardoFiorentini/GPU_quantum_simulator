...
__device__ void gate_2(float *vr, float *vi, int num_q, unitary Ur, unitary Ui, int target){
...
__device__ bool isIdentity(unitary *m_r, unitary *m_i){
...
bool isIdentityHost(unitary *m_r, unitary *m_i){
    return fabs(m_r->val[0]-1)<1e-3 && fabs(m_r->val[1])<1e-3 && fabs(m_r->val[2])<1e-3 && fabs(m_r->val[3]-1)<1e-3 && 
            fabs(m_i->val[0])<1e-3 && fabs(m_i->val[1])<1e-3 && fabs(m_i->val[2])<1e-3 && fabs(m_i->val[3])<1e-3;
}

__global__ void kernel(float *vr, float *vi, int num_q, unitary UrC, unitary UiC, int control, unitary UrT, unitary UiT, int target){
    if(!isIdentity(&UrC, &UiC)){
        gate_2(vr, vi, num_q, UrC, UiC, control);
    }
    if(!isIdentity(&UrT, &UiT)){
        gate_2(vr, vi, num_q, UrT, UiT, target);
    }
    cnot(vr, vi, num_q, control, target);
}
...
int main(int argc, char *argv[]){
    ...
    numBlocks = ceil((1LLU<<(num_q-1))/(double)NUMTHREAD);
    for(int i=0; i<num_q; i++){
        if(!isIdentityHost(&acc_r[i], &acc_i[i])){
            kernel_gate_2<<<numBlocks, NUMTHREAD>>>(
                d_state_vec_r,
                d_state_vec_i,
                num_q,
